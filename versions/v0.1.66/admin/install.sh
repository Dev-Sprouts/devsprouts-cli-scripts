...
DEFAULT_REPO="${CLI_INSTALL_REPO:-Dev-Sprouts/devsprouts-cli-scripts}"
DEFAULT_REF="${CLI_INSTALL_REF:-v0.1.66}"
DEFAULT_SCRIPT_DIR="${CLI_INSTALL_SCRIPT_DIR:-admin}"
BRAND_SLUG="${CLI_BRAND_SLUG:-devsprouts}"
BIN_NAME="${CLI_BIN_NAME:-devsprouts-admin}"
BRAND_NAME="${CLI_BRAND_NAME:-${BIN_NAME:-${BRAND_SLUG:-cli}}}"
CONFIG_DIR_NAME="${CLI_CONFIG_DIR_NAME:-devsprouts-admin}"
PACKAGE_SCOPE_DEFAULT="${CLI_PACKAGE_SCOPE:-@dev-sprouts}"
PACKAGE_NAME_DEFAULT="${CLI_PACKAGE_NAME:-devsprouts-admin-cli}"
...
SCRIPT_SOURCE_DIR="$DEFAULT_SCRIPT_DIR"
SETUP_SCRIPT_URL=""
...
Options:
  ...
  --script-dir <path>    Repo directory containing published scripts (default: scripts)
  ...
  Environment:
    CLI_INSTALL_SKIP_CONFIG_INIT=1   Skip config init entirely
    CLI_CONFIG_INIT_ARGS="..."       Extra args forwarded to `<bin> config init`
...
ensure_setup_script() {
  ...
  if [[ -z "$url" ]]; then
    ...
    url="https://raw.githubusercontent.com/${REPO_SLUG}/${REF_NAME}/${SCRIPT_SOURCE_DIR}/setup-private-registry.sh"
  fi
...
run_config_init() {
  ...
  local -a config_init_args=()

  if [[ "${CLI_INSTALL_SKIP_CONFIG_INIT:-}" == "1" ]]; then
    echo "Skipping '$cli_bin config init' because CLI_INSTALL_SKIP_CONFIG_INIT=1."
    return
  fi

  if [[ -n "${CLI_CONFIG_INIT_ARGS:-}" ]]; then
    # CLI_CONFIG_INIT_ARGS is intended for automation. URLs and flags are space-delimited.
    read -r -a config_init_args <<< "${CLI_CONFIG_INIT_ARGS}"
  fi
  ...
  if [[ -f "$config_path" ]]; then
    if [[ -t 0 ]]; then
      ...
      if [[ "$answer" =~ ^[Yy]$ ]]; then
        "$cli_bin" config init --force "${config_init_args[@]}"
      else
  ...
  if [[ ${#config_init_args[@]} -eq 0 && ! -t 0 ]]; then
    config_init_args=(--observability-vendor none)
    echo "Non-interactive install detected; initializing config with '--observability-vendor none'."
    echo "Re-run '$cli_bin config init' later with LGTM details when ready."
  fi

  "$cli_bin" config init "${config_init_args[@]}"
}
